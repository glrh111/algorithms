...
func TestBST(t *testing.T) {
	...
	Convey("测试基本内容", t, func() {
		...
		var st SortedSymbolTableInterface = NewBST()
		...
		fmt.Println(len(st.Keys()), st.Size())
		...
		// 测试 SelectKey 1) 边界的两个值 2) 超出边界的一个值 3) 中间的某个值 FIXME 这里有bug
		for _, value := range st.Keys() {
			fmt.Println(value.Value())
		}
		...
		So(err, ShouldEqual, nil)
		fmt.Println("selectedKey is : ", selectedKey)
